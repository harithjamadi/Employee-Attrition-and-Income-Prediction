...
# Choose number of clusters (e.g., 3 here)
kmeans_result <- kmeans(hr_scaled, centers = 3, nstart = 25)

# Add cluster labels to the original data
hr_cluster$Cluster <- as.factor(kmeans_result$cluster)

# Summary of clusters
table(hr_cluster$Cluster)
aggregate(hr_encoded, by = list(Cluster = kmeans_result$cluster), FUN = mean)

wss <- numeric(10)

for (k in 1:10) {
  kmeans_k <- kmeans(hr_scaled, centers = k, nstart = 10)
  wss[k] <- kmeans_k$tot.withinss
}
